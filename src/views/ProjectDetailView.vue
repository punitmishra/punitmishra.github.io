  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 pt-20">
    <div class="max-w-7xl mx-auto px-6 py-12">
      ...
      <div v-if="loading" class="text-center py-20">
        ...
        <p class="mt-4 text-gray-600 dark:text-gray-400">Loading project details...</p>
      </div>
      ...
      <div v-else-if="project" class="space-y-8">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-xl">
          <div class="flex items-start justify-between mb-6">
            <div class="flex-1">
              ...
              <p class="text-xl text-gray-600 dark:text-gray-400 mb-6">{{ project.description || "No description" }}</p>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-6 pt-6 border-t border-gray-200 dark:border-slate-700">
            <div>
              <div class="text-2xl font-black text-blue-600 dark:text-blue-400">{{ project.stargazers_count || 0 }}</div>
              <div class="text-sm text-gray-600 dark:text-gray-400">Stars</div>
            </div>
            <div>
              <div class="text-2xl font-black text-purple-600 dark:text-purple-400">{{ project.forks_count || 0 }}</div>
              <div class="text-sm text-gray-600 dark:text-gray-400">Forks</div>
            </div>
            <div>
              <div class="text-2xl font-black text-pink-600 dark:text-pink-400">{{ project.language || "N/A" }}</div>
              <div class="text-sm text-gray-600 dark:text-gray-400">Language</div>
            </div>
            <div>
              <div class="text-2xl font-black text-indigo-600 dark:text-indigo-400">{{ project.size || 0 }} KB</div>
        ...
        <!-- Recent Commits -->
        <div v-if="commits.length > 0" class="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-xl">
          <h2 class="text-3xl font-bold mb-6 text-gray-900 dark:text-white">Recent Activity</h2>
          <div class="space-y-4">
            <div
            ...
            >
              <BaseIcon :path="mdiCodeBraces" size="20" class="text-blue-600 dark:text-blue-400 mt-1" />
              <div class="flex-1">
                <p class="text-gray-900 dark:text-white font-medium mb-1">{{ commit.message.split('\n')[0] }}</p>
                <div class="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <a :href="commit.url" target="_blank" class="text-blue-600 dark:text-blue-400 hover:underline">
                    {{ commit.sha }}
                  </a>
                </div>
              </div>
